...
#
# Constantes e helpers ficam no TOPO do arquivo (fora dos blocos namespace):
# atribuir constante dentro de bloco é "dynamic constant assignment" em Ruby.

FINTURE_TERMINAL_STAGES = { 'ganho' => 'won', 'perdido' => 'lost' }.freeze

# ganho/perdido (etapa antiga) -> sdr_outcome; sdr_stage volta à última etapa real.
def finture_migrate_outcomes
  count = 0
  Conversation.where("custom_attributes ->> 'sdr_stage' IN ('ganho','perdido')").find_each do |conversation|
    ...
    outcome = FINTURE_TERMINAL_STAGES[attrs['sdr_stage']]
    history = attrs['sdr_history'].is_a?(Array) ? attrs['sdr_history'] : []
    closing = history.reverse.find { |entry| FINTURE_TERMINAL_STAGES.key?(entry['to']) }
...
def finture_backfill_transitions
  count = 0
  Conversation.where("custom_attributes -> 'sdr_history' IS NOT NULL").find_each do |conversation|
    ...
    history.each do |entry|
      count += 1 if finture_create_transition_from_history(conversation, entry)
    end
  end
  count
end

def finture_create_transition_from_history(conversation, entry)
  to = entry['to']
  terminal = FINTURE_TERMINAL_STAGES.key?(to)
  stage = terminal ? (entry['from'].presence || Finture::PipelineStage::DEFAULT_SLUG) : to
  ...
  Finture::StageTransition.create!(
    ...
    to_stage: stage,
    to_stage_label: stage,
    kind: terminal ? FINTURE_TERMINAL_STAGES[to] : 'stage_change',
    reason: entry['reason'],
...
namespace :finture do
  namespace :sdr do
    ...
    task backfill: :environment do
      converted = finture_migrate_outcomes
      transitions = finture_backfill_transitions
